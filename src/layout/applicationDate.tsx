...
export default function ApplicationDate({
  ...
  return (
    <div className={className}>
      <div
        className={`grid grid-cols-[4rem_1fr] md:grid-cols-[6rem_1fr] gap-0 bg-gray-3 text-center rounded-md w-full`}
      >
        {/* 왼쪽: 이번달(5월) 세로 병합 */}
        <div className="row-span-2 flex flex-col justify-center items-center text-0-75-500 md:text-0-875-500 bg-gray-50 border-r-[0.1rem] border-gray-2 w-[4rem] md:w-[6rem]">
          {isThisMonth ? '이번달' : '다음달'}
          <br />({month})
        </div>
        {/* 오른쪽: 전반기 */}
        ...
        >
          <div className="flex items-center justify-center p-0.5 md:p-1 text-0-75-500 text-gray-1 text-xs md:text-sm">
            전반기
          </div>
          <div
            className={`flex items-center justify-center p-0.5 md:p-1 h-[1.5rem] md:h-[2rem] text-0-75-500 md:text-0-875-500 text-xs md:text-sm ${
              showFirstHalfCheckbox && showSecondHalfCheckbox
                ? 'col-span-3'
                : 'col-span-2'
            }`}
